...
Confluence 일정 항목과 Jira 이슈의 매칭은 Gemini AI를 사용한다.
매칭 결과는 DB(`data/matches.db`)에 영구 저장되어, 동일한 일정/이슈 구성이면 Gemini를 다시 호출하지 않는다.

### 재매칭이 발생하는 경우

| 상황 | 동작 |
|------|------|
| 일정/이슈 변경 없음 | 저장된 매칭 재사용 (Jira 상태만 최신으로 갱신) |
| Confluence 일정 항목 추가/삭제 | 자동 재매칭 |
| Jira 이슈 추가/삭제 | 자동 재매칭 |
| 웹에서 새로고침 버튼 (`?refresh=1`) | 강제 재매칭 |

> Jira 이슈의 **상태 변경**(진행 중 → 완료 등)은 매칭에 영향을 주지 않으며, 항상 Jira에서 최신 상태를 가져온다.
